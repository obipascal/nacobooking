...
function sendMail($recipient, $type, $message) {
    // From URL to get webpage contents.
$url = "https://messenger.nacobooking.com/api/mailer/send";


$cURLConnection = curl_init();

$data = json_encode(['recipient' => $recipient, 'type' => $type, 'body' => $message]);
...
class Booking extends BaseModel
{
    ...
    public function getPaymentLink()
    {
        ...
        return url(app_get_locale(false,false , "/").config('booking.booking_route_prefix') . '/' . $this->code .'/complete-reservation');
    }
    ...
    {
        try {
            $adminMessage = (new NewBookingEmail($this, 'admin'))->render();
            $vendorMessage = (new NewBookingEmail($this, 'vendor'))->render();
            $customerMessage = (new NewBookingEmail($this, 'customer'))->render();
            
            // To Admin
            sendMail(setting_item('admin_email'), 'admin', $adminMessage);
            // to Vendor
            sendMail(User::find($this->vendor_id)->email, 'vendor', $vendorMessage);
            // To Customer
            sendMail($this->email, 'customer', $customerMessage);
    ...
    public function sendNewBookingEmailsToVendorAndAdmin()
    {
        try {
    ...
    public function sendBookingPaymentLink ()  {
          try {
              
            $customerMessage = (new NewBookingEmail($this, 'customer', $this->getPaymentLink()))->render();
           
            // To Customer
          return sendMail($this->email, 'customer', $customerMessage);
